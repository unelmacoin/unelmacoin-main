...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeunelmacoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeunelmacoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeunelmacoinAmount = strAnonymizeunelmacoinAmount.remove(strAnonymizeunelmacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeunelmacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeunelmacoinAmount * COIN) nMaxToAnonymize = nAnonymizeunelmacoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeunelmacoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeunelmacoinAmount));
        strAnonymizeunelmacoinAmount = strAnonymizeunelmacoinAmount.remove(strAnonymizeunelmacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeunelmacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeunelmacoinAmount == 0) {
